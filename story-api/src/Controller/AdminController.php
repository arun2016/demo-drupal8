...
/**
  @file
  Contains \Drupal\story\Controller\AdminController.
 */

namespace Drupal\story\Controller;
...
class AdminController extends ControllerBase {
    ...
    if (!empty($_FILES['multi_img']['name'])) {
      foreach ($_FILES['multi_img']['name'] as $k => $v) {
        ...
        if (!empty($response['error']))
          return;
        if (!empty($response['s3_response']['aws_error']))
          return;
  ...
  function videopage() {
    ...
    if ($_GET['vid'] == 12307) {
      ...
      $rdata[0]['body'] = "She recently posted a selfie of Shah Rukh Khan and son Aryan Khan and captioned it, 'Strike a pose ...т?дя╕?'.";
    ...
    elseif ($_GET['vid'] == 15041) {
      ...
      $rdata[1]['body'] = "She recently posted a selfie of Shah Rukh Khan and son Aryan Khan and captioned it, 'Strike a pose ...т?дя╕?'.";
